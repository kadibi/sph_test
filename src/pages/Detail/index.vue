        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 这里加入购物车进行路由跳转之前，发请求将你购买的产品信息通过请求的方式通知服务器，服务器进行相应的储存 -->
...
export default {
  ...
  mounted() {
    ...
    //派发actions获取产品详情信息
    this.$store.dispatch('getGoodInfo', this.$route.params.skuid)//这里是skuid是因为路由配置里面用的skuid给传进去的动态参数占得位
  },
  computed: {
    // 前面加了||计算出来的至少是个空对象
    ...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrList']),
    //给子组件的数据进行再计算，如果服务器的数据没有回来，skuinfo是个空对象，skuImgList至少要是个空数组
    skuImgList() {
  ...
  methods: {
    ...
    // 加入购物车的函数
    // 1.发请求---将产品的信息和数量加入到数据库（通知服务器）
    //2.服务器储存成功----进行路由的跳转，记得带参
    //3.请求失败---提醒用户
    async addShopCart(){
    try{
      await this.$store.dispatch('addShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})//这个函数调用之后返回一个promise对象
      //进行路由跳转
      // 简单的数据用query形式给路由组件传递过去
